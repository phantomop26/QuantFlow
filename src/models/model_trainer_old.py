...
class ModelTrainer:
    """Handles training and evaluation of different ML models"""
    ...
    def build_lstm_model(self, input_shape: Tuple[int, int]) -> keras.Model:
        ...
        try:
            model = keras.Sequential([
                ...
                layers.LSTM(units=MODEL_CONFIG.LSTM_UNITS // 4),
                layers.Dropout(MODEL_CONFIG.DROPOUT_RATE),
            ...
            self.logger.info(f"LSTM model built successfully. Input shape: {input_shape}")
            return model
    ...
    def build_gru_model(self, input_shape: Tuple[int, int]) -> keras.Model:
        ...
        try:
            model = keras.Sequential([
                ...
                layers.GRU(units=MODEL_CONFIG.LSTM_UNITS // 4),
                layers.Dropout(MODEL_CONFIG.DROPOUT_RATE),
            ...
            self.logger.info(f"GRU model built successfully. Input shape: {input_shape}")
            return model
    ...
    def train_lstm_model(self, X: np.ndarray, y: np.ndarray, 
                        model_name: str = "lstm_model") -> keras.Model:
        """
        Train LSTM model
    ...
    def train_gru_model(self, X: np.ndarray, y: np.ndarray, 
                       model_name: str = "gru_model") -> keras.Model:
        """
        Train GRU model
    ...
    def predict(self, model_name: str, X: np.ndarray, 
                confidence_interval: bool = False) -> np.ndarray:
        """
